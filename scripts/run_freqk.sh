#!/bin/bash -l
#SBATCH --job-name=freqk_gr
#SBATCH --output=/home/tbellagio/scratch/freqk_gr/logs/freqk_%x_%j.out
...
# =============================================================================
# run_freqk.sh
# Run the full freqk pipeline for a single GrENE-Net sample.
#
# Arguments (passed by launch.py via sbatch):
#   $1  SAMPLE_ID   e.g. MLFH040320180306
...
SAMPLE_ID=${1:?Usage: run_freqk.sh SAMPLE_ID R1 R2 [K]}
R1=${2:?}
R2=${3:?}
K=${4:-31}
...
# Parse site from sample ID: MLFH + site(2) + ...
SITE=${SAMPLE_ID:4:2}

OUT_DIR=${RESULTS}/site${SITE}/${SAMPLE_ID}/k${K}
mkdir -p "${OUT_DIR}"
